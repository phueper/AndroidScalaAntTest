...
/*	implicit def func2OnItemSelectedListener0(funcs:(()=>Unit, ()=>Unit)) = {
		new OnItemSelectedListener() {
			def onItemSelected(p:AdapterView[_], v:View, pos:int, id:long) = funcs._1.apply
			def onNothingSelected(p:AdapterView[_]) = funcs._2.apply
		}
	}
*/
...
/*		spinner.setOnItemSelectedListener( ()=>{textView.append("item selected\n");scrollViewUpdate(scrollView)}, 
				()=>{textView.append("nothing selected\n");scrollViewUpdate(scrollView)})
*/
